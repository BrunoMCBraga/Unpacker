...
class CreateProcessA:
    ...
    def get_arguments_list(vivisect_engine):
        ...
        if lp_application_name != 0x0:
            ...
            application_name = MemoryDataInterpreter.bytearray_to_ascii_string(application_name_bytes)

        command_line = None

        if lp_command_line != 0x0:
            command_line_bytes = vivisect_engine.read_memory(lp_command_line, CreateProcessA.DEFAULT_READ_SIZE)
            command_line = MemoryDataInterpreter.bytearray_to_ascii_string(command_line_bytes)
        ...
        if lp_current_directory != 0x0:
            ...
            current_directory = MemoryDataInterpreter.bytearray_to_ascii_string(current_directory_bytes)
